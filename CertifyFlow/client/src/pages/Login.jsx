...
import { ShieldCheck, Mail, Lock, User, ArrowRight, Leaf, Wifi } from 'lucide-react';
...
const Login = () => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-[#EAF2EF]" style={{ background: 'linear-gradient(135deg, #EAF2EF 0%, #D8E5DF 100%)' }}>
            ...
            >
                {/* Visual Side (Left) - Green Aesthetic */}
                <div className="w-full md:w-1/2 p-12 relative flex flex-col justify-between overflow-hidden bg-[#739E82]" style={{ background: 'linear-gradient(135deg, #8FB9AA 0%, #6FA295 100%)' }}>
                    ...
                    >
                        <div className="flex items-center gap-3 mb-10">
                            ...
                            <span className="text-2xl font-bold text-white tracking-wide">CertifyFlow</span>
                        </div>

                        <h1 className="text-4xl md:text-5xl font-bold text-white mb-6 leading-tigher drop-shadow-sm">
                            Verify <br /> with Confidence
                        </h1>
                        <p className="text-[#EAF7F2] text-lg leading-relaxed max-w-sm opacity-90 font-medium">
                            Access your certificates securely. The most trusted platform for academic and professional verification.
                        </p>
                    </motion.div>
                    ...
                    <motion.div
                        ...
                        className="relative z-10 mt-12 bg-white/10 backdrop-blur-md p-6 rounded-3xl border border-white/10 shadow-lg hover:bg-white/15 transition-all cursor-pointer group"
                    >
                        <div className="flex items-center gap-4 mb-4">
                            <div className="w-12 h-12 rounded-full bg-[#EAF7F2] flex items-center justify-center shadow-md group-hover:scale-110 transition-transform">
                ...
                <div className="w-full md:w-1/2 bg-white flex flex-col justify-center relative">
                    {/* Animated white curve transition */}
                        ...
                        >
                            <div className="mb-8">
                                ...
                                <p className="text-[#5F7A74] font-medium">
                                    {isRegistering ? 'Join us to secure your future.' : 'Please enter your details to sign in.'}
                                </p>
                            </div>

                            {/* Role Toggle */}
                            <div className="flex bg-[#F1F5F4] p-1.5 rounded-2xl mb-8 border border-[#E0EBE6]">
                                <button
                                    onClick={() => setRole('student')}
                                    className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-300 ${role === 'student'
                                            ? 'bg-white text-[#2F3E3A] shadow-md transform scale-[1.02]'
                                            : 'text-[#6FA295] hover:text-[#5F9487]'
                                        }`}
                                >
                                    Student
                                </button>
                                <button
                                    ...
                                    className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-300 ${role === 'admin'
                                            ? 'bg-[#6FA295] text-white shadow-md transform scale-[1.02]'
                                            : 'text-[#6FA295] hover:text-[#5F9487]'
                                        }`}
                                >
                                    Admin
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-1">
                                    <div className="flex justify-between items-center ml-1 mb-1.5">
                                        ...
                                        {!isRegistering && (
                                            <button type="button" className="text-xs text-[#6FA295] hover:text-[#5F9487] font-bold hover:underline">
                                                Forgot Password?
                                            </button>
                                        )}
                                    </div>
                                ...
                                <motion.button
                                    ...
                                    type="submit"
                                    disabled={isLoading}
                                    className="w-full text-white font-bold py-4 rounded-2xl shadow-xl shadow-[#6FA295]/30 transition-all flex items-center justify-center gap-2 mt-6 overflow-hidden relative"
                                    style={{
                                        background: 'linear-gradient(135deg, #8FB9AA 0%, #6FA295 100%)',
                                    }}
                                >
                                    {isLoading ? (
